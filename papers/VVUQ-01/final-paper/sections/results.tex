...
\centering
\begin{tabularx}{\textwidth}{L{6.7cm} >{\raggedright\arraybackslash}X}
\toprule
...
\midrule
  ...
  \texttt{execution/03-vvuq/} : \texttt{README.md} & Examples 1 to 3, the 6 case gate decision surface. \\
...
to one onto the source packages, as shown in \autoref{tab:results_tests}. Alongside the tests, the three static
...
\autoref{figwaterfall}. The generation itself was effectively instantaneous: on a representative
deliverable the four artifacts measured 963, 735, 95, and 1000 bytes for the
instruction set, the generated code, the execution log, and the paper, the
slowest of the four stages completed in under two ten thousandths of a second,
and the largest artifact, the 1000 byte paper, was far below the 200{,}000 byte
cap. Three artifacts from the pipeline are
...
producing a well formed candidate is cheap, so the value of the workflow is not there.
...
\begin{figure}[H]
    \hspace{-0.8cm}%
      {\includegraphics[width=1.1\linewidth, trim={0 25 0 0}, clip]{Images/02-acceleration-waterfall.png}}
    \vspace{-0.5cm}
    \caption{Schedule acceleration waterfall from 30 days to 12 days automated schedule, a 2.5x reduction.}
    \label{figwaterfall}
\end{figure}
...
0.10 bound, and the candidate was accepted. The two validated metrics carried
relative errors of 0.020 for the acceleration factor and 0.033 for the automated
day count, both inside the 0.05 bound, and the low dispersion accept showed an
acceleration factor mean of 2.5033 with a standard deviation of 0.0125 and an
automated day count mean of 12.0000 with a standard deviation of 0.0816. 

The
escalated case in the table below is the same measurement under deliberately
divergent runs, where the automated day count coefficient of variation rose from
0.0068 to 0.3646, well past the bound, which is what triggered the escalation
rather than a silent block. The accepted deliverable clears every spoke of the
threshold ring shown in \autoref{figwheel}. The decisive evidence,
however, is the full decision surface in \autoref{tab:results_gate}, where the
gate was exercised across six cases. 

It accepted exactly one and blocked five,
...
\centering
\begin{tabularx}{\textwidth}{C{0.6cm} L{5.7cm} C{1.3cm} C{1.3cm} >{\raggedright\arraybackslash}X}
\toprule
...
divergent. The per run values, run under three distinct seeds, are given in
\autoref{tab:results_triplerun}; the run to run spread is tiny because the
generation is deterministic in the run count, a point taken up honestly in the
discussion and the limitations. Robust ingestion degraded gracefully: with the network unreachable the
...
Autochunking was verified to be lossless: the chunker is byte
identical on line boundary splits and on ASCII hard splits, both independently
reconstructed and confirmed. In the worked example a 114{,}890 byte document
split into six chunks under a reduced 20{,}000 byte cap and reconstructed byte for
byte, and a separate oversized document split into four chunks, each carrying a
reconstruction README that records the exact concatenation command. One genuine limitation was discovered here and is
...
characters in the tested case, because of a decode that can ignore errors. The scheduler planned twenty four evenly spaced commit slots at
an hourly interval of 3600 seconds. These outcomes connect the platform to the
prior triplicate and digital twin work that motivated triple runs~\cite{kawchak2025pdactwin}.

\begin{table}[H]
\caption{Triple run metrics and consensus across three seeds.}
\label{tab:results_triplerun}
\centering
\begin{tabularx}{\textwidth}{L{2.8cm} C{2.8cm} C{3.0cm} C{3.0cm} >{\raggedright\arraybackslash}X}
\toprule
\textbf{Run} & \textbf{Seed} & \textbf{Accel. factor} & \textbf{Automated days} & \textbf{Divergent} \\
\midrule
  Run 1          & 20260521 & 2.502  & 11.998  & no \\
  Run 2          & 20260522 & 2.504  & 11.996  & no \\
  Run 3          & 20260523 & 2.506  & 11.994  & no \\
  \midrule
  Consensus mean & -        & 2.5040 & 11.9960 & no \\
  CV across runs & -        & 0.0007 & 0.0001  & within bound \\
\bottomrule
\end{tabularx}
\end{table}
...
Among the four default interlocks is
...
the generator, is the component that scales across both stages. The four safety
cases exercised on the controller are summarized in
\autoref{tab:results_factory}, covering a clean batch, an over budget fault, an
unsatisfied interlock, and a within budget fault, each ending in the expected
state.

\begin{table}[H]
\caption{Lights off factory safety cases and their outcomes.}
\label{tab:results_factory}
\centering
\begin{tabularx}{\textwidth}{C{1.0cm} L{7.0cm} C{1.9cm} C{2cm} >{\raggedright\arraybackslash}X}
\toprule
\textbf{Case} & \textbf{Scenario} & \textbf{End state} & \textbf{Faults} & \textbf{Blocked} \\
\midrule
  A & clean batch, 6 tasks, fault budget 1 & dark & 0 & no \\
  B & over budget, 9 tasks, every 3rd faults & estop & 2 & no \\
  C & interlock unsatisfied (perimeter) & fault & 0 & yes \\
  D & within budget, 9 tasks, fault budget 3 & dark & 3 & no \\
\bottomrule
\end{tabularx}
\end{table}
...
and the whole run spans roughly eighteen minutes. 
...
% \autoref{tab:results_figs}~\cite{sel2025vvuq}.
...
\centering
\begin{tabularx}{\textwidth}{L{5.5cm} >{\raggedright\arraybackslash}X}
\toprule
\textbf{Script directory \texttt{(imagegen/})} & \textbf{Renders as} \\
\midrule
