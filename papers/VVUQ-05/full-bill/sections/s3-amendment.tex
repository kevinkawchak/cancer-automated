...
%% Rendered visuals: Table 2 (ten-gate threshold schedule), Figure 3 (gate
%% decision rule and funnel), Figure 4 (statutory layering through Title 21),
...
\hline
\end{xltabular}}
\figcaption{Table 2. Ten-gate threshold schedule: the minimum validation
agreement, the maximum coefficient of variation, the hard catastrophe predicate,
and the governing external standards for each gate. Gates 06, 09, and 10 carry a
hard predicate at agreement 1.00.}

\uscprov{1}{The decision rule and the gate funnel that this schedule implements
are shown in Figure 3.}

\begin{asciifig}
DECISION RULE (per interaction)    TEN-GATE FUNNEL (candidates to ACCEPT)
+------------------------------+   ===================================== 100%
| Verify  : pass fraction 1.0  |    [01] handeye-servo      V>=0.97 CV<=0.08
|          (built it right?)   |    [02] finger-force       V>=0.95 CV<=0.10
| Validate: agree >= gate min  |    [03] whole-body-balance V>=0.98 CV<=0.06
|          max rel err<=bound  |    [04] plan-correctness   V>=0.95 CV<=0.10
|          human review kept   |    [05] grasp-handover     V>=0.96 CV<=0.10
| Quantify: CV over >=3 runs   |    [06] vascular-no-fly  * V=1.00  CV<=0.05
|          <= per-gate bound   |    [07] suturing-anastom.  V>=0.96 CV<=0.08
+------------------------------+    [08] perception-scene   V>=0.95 CV<=0.10
             |                      [09] human-collision  * V=1.00  CV<=0.06
             v                      [10] fault-estop      * V=1.00  CV<=0.05
  ACCEPT  -> all dimensions pass                |
  BLOCK   -> any dimension fails                v
  ESCALATE-> divergence to human    ACCEPT (all 10) -> composite reported
                                    * = hard catastrophe predicate gate
\end{asciifig}
\figcaption{Figure 3. The VVUQ gate decision rule over the verify, validate, and
quantify dimensions, beside the ten-gate funnel a candidate interaction must
clear; the three starred gates carry a hard catastrophe predicate.}
...
audit trail' have the meanings given in the prior bill and reconciled with section
201(h) and section 520(o) of this Act.}
...
Figure 4 shows where the new section attaches on the device-regulation pathway,
and Table 3 is the conforming-amendment crosswalk.}
...
\begin{asciifig}
  ...
  Is it a "device"?  .................  sec. 321(h)          (s321)
        ...
        |  autonomous robot-control software, sec. 360j(o)  (s360j)
        v
  Classify the device  ..............   sec. 360c           (s360c)
        |                                  |
  Class II -> 510(k) notice  ........   sec. 360(k)         (s360)
  Class III -> premarket approval ...   sec. 360e           (s360e)
        |                                  |
        +--------> Predetermined change -> sec. 360e-4       (s360e-4)
                   control plans (PCCP) ==> NEW sec. 360e-5  (verify before
  ...
  Cross-cutting duties the new section leans on:
   - Adulteration / quality (QMSR)  ..   sec. 351            (s351)
   - Prohibited acts / enforcement  ..   sec. 331            (s331)
   - Real-world evidence for trials  .   sec. 355g           (s355g)
   - State preemption boundary  ......   sec. 360k           (s360k)
   - Short title of the whole Act  ...   sec. 301            (s301)
\end{asciifig}
\figcaption{Figure 4. Statutory layering of new section 360e-5 through Title 21:
the device question, the clinical-decision-support carve-out it must not fall
through, classification, the 510(k) and premarket-approval pathways, the section
360e-4 keystone, and the new mandatory, ex ante section 360e-5.}
...
executes robot-patient interaction code\\
\hline
...
verification record\\
\hline
(8) & \S~515C & \S~360e-4 & Change control plan must include an automated
...
\hline
\end{xltabular}}
\figcaption{Table 3. The ten conforming amendments crosswalk: each amendment by
its Federal Food, Drug, and Cosmetic Act section, its 21 U.S.C. citation, and the
change H. R. 9510 makes. The marked text appears in the comparative print in
section 4.}
